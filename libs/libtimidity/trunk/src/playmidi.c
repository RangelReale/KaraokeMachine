...
static void evt_note_on(MidSong *song, MidEvent *e)
{
...
{
  evt_note_on(song, song->current_event);
}
...
static void evt_note_off(MidSong *song, MidEvent *e)
{
  int i = song->voices;
  //MidEvent *e = song->current_event;
...
{
  evt_note_off(song, song->current_event);
}

/* Process the All Notes Off event */
static void evt_all_notes_off(MidSong *song, MidEvent *event)
{
  int i = song->voices;
  int c = event->channel;
...
{
  evt_all_notes_off(song, song->current_event);
}

/* Process the All Sounds Off event */
static void evt_all_sounds_off(MidSong *song, MidEvent *event)
{
  int i = song->voices;
  int c = event->channel;
...
{
  evt_all_sounds_off(song, song->current_event);
}

static void evt_adjust_pressure(MidSong *song, MidEvent *e)
{
  //MidEvent *e = song->current_event;
  int i = song->voices;
...
{
  evt_adjust_pressure(song, song->current_event);
}

static void evt_drop_sustain(MidSong *song, MidEvent *event)
{
  int i = song->voices;
  int c = event->channel;
...
{
  evt_adjust_volume(song, song->current_event);
}
...
{
  if (!song->groomed_event_count) return 0;
...
void mid_song_event (MidSong *song, MidEvent *event)
{
  ...
  //samples = size / song->bytes_per_sample;
	...
	if (!(event->b)) /* Velocity 0? */
        ...
        case ME_KEYPRESSURE:
          evt_adjust_pressure(song, event);
          break;

          /* Effects affecting a single channel */

        case ME_PITCH_SENS:
          song->channel[event->channel].pitchsens = event->a;
          song->channel[event->channel].pitchfactor = 0;
          break;

        case ME_PITCHWHEEL:
          song->channel[event->channel].pitchbend =
            event->a + event->b * 128;
          song->channel[event->channel].pitchfactor = 0;
        ...
        case ME_MAINVOLUME:
          song->channel[event->channel].volume =
	    event->a;
          evt_adjust_volume(song, event);
          break;

        case ME_PAN:
          song->channel[event->channel].panning =
	    event->a;
          break;

        case ME_EXPRESSION:
          song->channel[event->channel].expression =
	    event->a;
          evt_adjust_volume(song, event);
          break;
		...
		song->channel[event->channel].bank = event->a;
          ...
          {
              /* Mark this instrument to be loaded */
              if (!(song->tonebank[0]->instrument[event->a]))
              {
                song->tonebank[0]->instrument[event->a] = MAGIC_LOAD_INSTRUMENT;
                load_missing_instruments(song);
              }

            song->channel[event->channel].program = event->a;
          }
          break;

        case ME_SUSTAIN:
          song->channel[event->channel].sustain =
	    event->a;
          if (!event->a)
            evt_drop_sustain(song, event);
          break;

        case ME_RESET_CONTROLLERS:
          reset_controllers(song, event->channel);
          break;

        case ME_ALL_NOTES_OFF:
          evt_all_notes_off(song, event);
          break;

        case ME_ALL_SOUNDS_OFF:
          evt_all_sounds_off(song, event);
          break;

        case ME_TONE_BANK:
          song->channel[event->channel].bank =
	    event->a;
          break;
...
size_t mid_song_read_wave(MidSong *song, void *ptr, size_t size)
{
  ...
  } else { //song->groomed_event_count
    compute_data(song, (sint8 **)&ptr, samples);
  } //song->groomed_event_count
  return samples * song->bytes_per_sample;
}
